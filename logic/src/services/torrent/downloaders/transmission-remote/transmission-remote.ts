    ...
    try {
      ...
      if (error instanceof Error) {
        if (TransmissionRemoteErrorsManager.isUnrecognizedTorrentError(error)) {
          const stdout = (error as unknown as { stdout: Buffer }).stdout.toString();
          const stderr = (error as unknown as { stderr: Buffer }).stderr.toString();

          // TODO: test how the metadata looks in the log from the errors
